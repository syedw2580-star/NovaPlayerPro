      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        <div className="space-y-2.5">
          ...
          {activeVideoBookmarks.length === 0 ? (
            <div className="bg-white/5 p-4 rounded border border-white/10 text-center text-white/40 text-xs shadow-inner">
              <p>No bookmarked scenes for this video.</p>
              <p className="text-[10px] mt-1 text-white/30">Tap the Bookmark flag [ ] on the player controls to pin current seconds with title text.</p>
            </div>
          ) : (
        ...
        <div className="space-y-2.5">
          ...
          <div className="bg-white/5 p-3 rounded border border-white/10 space-y-2 text-xs">
            <div className="grid grid-cols-12 gap-y-2 pb-1 text-[9px] border-b border-white/10 font-bold uppercase tracking-wider text-white/40 font-mono">
            ...
            {shortcuts.map((sc, i) => (
              <div key={i} className="grid grid-cols-12 items-center text-[11px] py-1">
                ...
                <span className="col-span-7 text-white/60">
                  {sc.action}
                </span>
              </div>
            ))}
